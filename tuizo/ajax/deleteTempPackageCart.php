...
if (isset($_POST['package_id'])) {
    ...
    $query_of_deleteTempPackageCart = "DELETE FROM package_temp_cart WHERE PTC_package_id=" . intval($package_id) . " AND PTC_session_id='" . $session_id . "'";
    
    $result_of_deleteTempPackageCart = mysqli_query($con, $query_of_deleteTempPackageCart);
    if ($result_of_deleteTempPackageCart) {
        $query_of_deleteTempProductCart = "DELETE FROM product_temp_cart WHERE ProTC_PTC_package_id=" . intval($package_id) . " AND ProTC_session_id='" . $session_id . "'";
        $result_of_deleteTempProductCart = mysqli_query($con, $query_of_deleteTempProductCart);
        if ($result_of_deleteTempProductCart) {
            /** start: query for package by session id * */
            $query_of_packages_by_session = "SELECT
            ...
            while ($rows = mysqli_fetch_object($result_package_by_session)) {
                $package_carts[] = $rows;
            }
