...
if($Product_temp_cart_id > 0){
	
	$DeleteProduct = mysqli_query($con,"DELETE FROM product_temp_cart WHERE ProTC_id='$Product_temp_cart_id'");
	
	if($DeleteProduct){
		if(GetPackageQuantity($Package_ID) == 0){
			
			$GetAllPackageInfo = mysqli_query($con,"SELECT * FROM package_temp_cart WHERE PTC_session_id='$cart_id'");
			...
			//getting total individual package quantity
			...
			//getting all package information from package_temp_cart db
			...
			$data=array("output_type"=>2, "package_quantity"=>$NewPackageCount, "package_price"=>number_format((float)$TotalPackagePrice,2,'.',''), "total_temp_cart_price"=>number_format((float)$TotalPrice,2,'.',''), "total_temp_cart_discount"=>number_format((float)$TotalDiscount,2,'.',''),"full_cart"=>$total_cart, "total_package"=>$TotalPackageQuantity); // This is your data array/result
			echo json_encode($data);
	...
	$data=array("output_type"=>3, "product_quantity"=>$CurrentProductQuantity);
	echo json_encode($data);*/
}
